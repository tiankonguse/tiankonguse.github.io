...
categories: [算法]
updateData: 2025-06-15 12:13:00
published: true
---
...
规则：  
1）所有单词按驼峰命名，即首字母大写，其他单次都需要小写，但第一个单词的首字母需要小写。  
2）移除所有非字母字符。  
3）第一个字符为 `#`  
4）最长100个字符。  
...
按题意，第一步标题拆分为若干个字符串，拆过过程中把所有非字母字符移除。  
第二步，把每个单词的首字母大写，其他小写。  
第三步，把所有单词拼接起来，第一个单词的首字母小写。  
第五步，添加 `#` 号。  
第六步，判断长度是否超过100，超过了截断。  
...
由于是首尾乘积，所以中间的值都无所谓。  
由于是子序列，所以两个位置的距离只要不小于m，都可以当做首尾来计算答案。  
...
为了避免出错，工程师建议遵循职责单一原则。  
具体来说，每个变量的含义是固定的，不要使用变量的特殊值来做特殊判断。  
这里每个符号我都定义了最大值、最小值、是否有值。  
最大值和最小值默认定义为值域外的值，这样进行 max 与 min 时可以保证一定不会出错。  
...
有向路径中位节点定义：路径上具体起点最近的节点，且满足到起点的距离大于等于路径总距离的一半。  
...
后面意识到，两个二分的目标不一样，是无法二分的，只能写两个二分了。  
